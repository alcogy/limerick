...
<div class="page">
	...
	<div class="invoice-sheet">
		<!-- Header: left = invoice number + status + buyer, right = issuer + issue date + due date -->
		<div class="inv-header">
			...
			<div class="inv-right">
				...
				<div class="inv-meta">
					<div class="meta-row">
						<span>{t().invoice.issuedAt}</span><span>{formatDate(inv.issued_at)}</span>
					</div>
					<div class="meta-row">
						<span>{t().invoice.dueDate}</span><span>{formatDate(inv.due_date)}</span>
					</div>
				</div>
			</div>
		</div>
		...
		<table class="inv-table">
			...
			<tbody>
				{#each allItems as item (item.id)}
					<tr>
						<td
							>{item.name}
							<div class="item-sku">{item.sku}</div></td
						>
			...
			<tfoot>
				<tr
					><td colspan="4" class="num">{t().invoice.subtotal}</td><td class="num"
						>{formatCurrency(inv.subtotal)}</td
					></tr
				>
				<tr
					><td colspan="4" class="num">{t().invoice.taxAmount}</td><td class="num"
						>{formatCurrency(inv.tax_amount)}</td
					></tr
				>
				<tr class="total-row"
					><td colspan="4" class="num">{t().invoice.totalAmount}</td><td class="num"
