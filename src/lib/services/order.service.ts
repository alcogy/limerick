...
export async function advanceOrderStatus(
	...
	await writeAuditLog({ db: env.DB, user_id: user?.id ?? null, action: 'update', resource_type: 'order', resource_id: id, metadata: { status: to }, request });
	return { success: true };
}
...
export async function cancelOrder(ctx: ServiceCtx, id: string) {
	...
	await writeAuditLog({ db: env.DB, user_id: user?.id ?? null, action: 'cancel', resource_type: 'order', resource_id: id, metadata: { previous_status: order.status }, request });
	return { success: true };
}
